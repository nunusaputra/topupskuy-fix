...
const DetailContent = ({ data, product, attributes, myItems, payment, token }) => {
  ...
  const [selected, setSelected] = useState({
    ...
    zoneInputFF: groupedFields[1].ffId
  });
  ...
  if (groupedFields.length > 1) {
    isDisabled = !selected.price || !selected.itemId || !selected.phone || !selected.paymentName || !selected.userId || !selected.zoneId;
  } else if (groupedFields.length === 1) {
    isDisabled = !selected.price || !selected.itemId || !selected.phone || !selected.paymentName || !selected.userId;
  } else {
    isDisabled = !selected.price || !selected.itemId || !selected.phone || !selected.paymentName;
  }
  ...
  const handleModalSummary = () => {
    ...
    let url = `${API_URL}/my-product/check-id/${selected.productId}/${selected.userId}/${selected.zoneId}`;
    if (selected.zoneId === "") {
      url = `${API_URL}/my-product/check-id/${selected.productId}?user_id=${encodeURIComponent(selected.userId)}`
    }
    axios.get(url, {
    ...
    <>
        ...
        <div className="flex flex-col gap-5 lg:overflow-auto lg:sticky lg:top-32">
            ...
            <div className="w-full bg-slate-800 min-h-[7.5rem] flex flex-col gap-2 rounded-lg overflow-hidden">
              <div className="mb-5">
                ...
                <div className="text-white px-4 text-sm">
                  <ol className="list-decimal space-y-2 pl-5 mt-5" dangerouslySetInnerHTML={{ __html: product.description }}>
                  </ol>
                </div>
              </div>
            </div>
          ...
          <div className="w-full min-h-20 bg-slate-800 rounded-lg flex flex-col gap-2 px-4 py-3">
            <div className="w-full h-20 flex gap-4 items-center">
              ...
              <div className="w-full h-14 flex flex-col justify-center">
                <p className="text-white text-sm">{selected.item ? selected.item : "Belum dipilih"}</p>
              </div>
            </div>
            {groupedFields.length > 1 ? (
              <div className="w-full min-h-[5.5rem] flex flex-col gap-2">
                <div className="flex justify-between">
                  ...
                  <p className="text-white text-md">{selected.userId ? selected.userId : "-"}</p>
                </div>
                <div className="flex justify-between">
                  <h1 className="text-white text-md font-semibold">Zone</h1>
                  <p className="text-white text-md">{selected.zoneId ? selected.zoneId : "-"}</p>
                </div>
            ...
            <div className="w-full min-h-[5.5rem] flex flex-col gap-2">
              <div className="flex justify-between">
                ...
                <p className="text-white text-md">{selected.phone ? `+62${selected.phone}` : "-"}</p>
              </div>
              <div className="flex justify-between">
                <h1 className="text-white text-md font-semibold">Harga</h1>
                <p className="text-white text-md">{selected.price ? new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(selected.price) : "Rp 0"}</p>
              </div>
              <div className="flex justify-between">
                <h1 className="text-white text-md font-semibold">Payment Fee</h1>
                <p className="text-white text-md">{selected.feePayment ? new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(selected.feePayment) : "Rp 0"}</p>
              </div>
              <div className="flex justify-between">
                <h1 className="text-white text-md font-semibold">Pembayaran</h1>
                <p className="text-white text-md">{selected.paymentName ? selected.paymentName : "Belum dipilih"}</p>
              </div>
              <hr className="text-slate-600/60" />
            </div>
            <div className="w-full h-14 flex items-center justify-between">
              <h1 className="text-lg text-white font-bold">Total Pembayaran</h1>
              <p className="text-orange-400 text-md font-bold">{selected.price !== null || selected.feePayment !== null ? new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(Number(selected.price) + Number(selected.feePayment)) : "Rp 0"}</p>
            </div>
          </div>

          <button
            className={`w-full py-2 ${isDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-seventh"} text-white font-semibold shadow-md shadow-slate-900 rounded-lg flex items-center justify-center gap-2`}
            disabled={isDisabled}
      ...
      <div className="w-full relative lg:w-[65%] mt-6 lg:mt-0 flex flex-col gap-10">
        {data.length > 0 && data?.map((item, index) => {
          return (
            <>
              <div key={index} className="w-full min-h-30 bg-slate-800 rounded-lg ring-2 ring-slate-500 shadow-md shadow-slate-900 p-4 flex flex-col gap-4">
                {item.userInput.id === 1 && item.label !== "" ? (
                  <div key={index} className="flex items-center gap-2">
                    {index === 0 ? (<TbCircleNumber1Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) :
                      index === 1 ? (<TbCircleNumber2Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) :
                        index === 2 ? (<TbCircleNumber3Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) :
                          index === 3 ? (<TbCircleNumber4Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) :
                            index === 4 ? (<TbCircleNumber5Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) : null}
                    <h1 className="text-xl text-white font-semibold">{item.label}</h1>
                  </div>
                ) : (
                  <div key={index} className="flex items-center gap-2">
                    {index === 1 ? (<TbCircleNumber1Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) :
                      index === 2 ? (<TbCircleNumber2Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) :
                        index === 3 ? (<TbCircleNumber3Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) :
                          index === 4 ? (<TbCircleNumber4Filled className="text-2xl text-orange-500 ring-2 ring-white/70 rounded-full" />) : null}
                    <h1 className="text-xl text-white font-semibold">{item.label}</h1>
                  </div>
                )}
                        ...
                        {item.element === "Input" ? (
                          ...
                          (
                            <div className="flex flex-col gap-1">
                              ...
                              >
                                <option key={index} value="">{item.placeholder}</option>
                                {Array.isArray(item.datas)
                                  ...
                                  : Array.isArray(JSON.parse(item.datas))
                                    ? JSON.parse(item.datas).map((option, index) => (
                                      <option key={index} value={option.value}>
                                        {option.text}
                                      </option>
                                    ))
                ...
                {item.userInput.id === 2 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 2xl:grid-cols-4 gap-5">
                    ...
                    hover:bg-seventh ${selected.itemId === item.id
                        ...
                        key={item.id}
                        onClick={() => setSelected({ ...selected, itemId: item.id, item: item.name, price: item.sellPrice })}
                      >
                ...
                {selected.price != null && item.userInput.id === 3 ? (
                  <>
                    <div className="w-full min-h-10 bg-fourth/30 backdrop-blur-xl rounded-lg border border-slate-600 flex flex-col overflow-hidden">
                      <div className="flex justify-between items-center px-4 py-2" onClick={() => handleShow(1)}>
                        <h1 className="text-sm text-white font-semibold">Saldo</h1>
                        <span className={`text-lg text-white transform transition-transform duration-300 ${show === 1 ? "rotate-180" : "rotate-0"}`}>
                              ...
                              .map((value) => (
                                <div
                                  className={`w-full h-auto ring-offset-secondary/80 rounded-lg flex flex-col lg:flex-row items-center p-4 justify-center gap-4 hover:cursor-pointer ${selected.payment === value.id
                                    ...
                                    : "bg-white"
                                    }`} key={value.id} onClick={() => setSelected({ ...selected, payment: value.id, paymentName: value.name, feePayment: (selected.price * (value.feePercent / 100) + value.feeFlat).toFixed(2) })}>
                                  <div className={`w-24 h-16 lg:w-50 lg:h-18 overflow-hidden flex justify-center ${selected.payment === value.id ? "bg-white p-1 rounded-md" : ""}`} >
                                    <img
                                      src={value.icon.name}
                                      alt=""
                                  ...
                                  <div className="w-full lg:w-70 flex flex-col justify-center text-center lg:text-left">
                                    <h1 className={`text-lg font-semibold ${selected.payment === value.id ? "text-white" : ""}`} >
                                      {value.name}
                                    </h1>
                                    {selected.price < value.minAmount ? (
                                      <p className={`text-xs text-red-600 ${selected.payment === value.id ? "text-red-200" : ""}`} >
                                        Tidak Tersedia.{" "}
                                        <span className="block">Minimal {value.minAmount}</span>
                                      </p>
                                    ) : ""}
                                    <h1 className={`text-sm font-semibold ${selected.payment === value.id ? "text-white" : ""}`} >
                                      {new Intl.NumberFormat("id-ID", {
                                        style: "currency",
                                        currency: "IDR",
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2
                                      }).format(selected.price + (selected.price * (value.feePercent / 100) + value.feeFlat))}
                                    </h1>
                                  </div>
                                </div>
                              ))}
                    ...
                    <div className="w-full min-h-10 bg-fourth/30 backdrop-blur-xl rounded-lg border border-slate-600 flex flex-col overflow-hidden">
                      <div className="flex justify-between items-center px-4 py-2" onClick={() => handleShow(2)}>
                        <h1 className="text-sm text-white font-semibold">QRIS</h1>
                        <span className={`text-lg text-white transform transition-transform duration-300 ${show === 2 ? "rotate-180" : "rotate-0"}`}>
                    ...
                    <div className="w-full min-h-10 bg-fourth/30 backdrop-blur-xl rounded-lg border border-slate-600 flex flex-col overflow-hidden">
                      <div className="flex justify-between items-center px-4 py-2" onClick={() => handleShow(3)}>
                        <h1 className="text-sm text-white font-semibold">E-Wallet</h1>
                        <span className={`text-lg text-white transform transition-transform duration-300 ${show === 3 ? "rotate-180" : "rotate-0"}`}>
                    ...
                    <div className="w-full min-h-10 bg-fourth/30 backdrop-blur-xl rounded-lg border border-slate-600 flex flex-col overflow-hidden">
                      <div className="flex justify-between items-center px-4 py-2" onClick={() => handleShow(4)}>
                        <h1 className="text-sm text-white font-semibold">Virtual Account</h1>
                        <span className={`text-lg text-white transform transition-transform duration-300 ${show === 4 ? "rotate-180" : "rotate-0"}`}>
                    ...
                    <div className="w-full min-h-10 bg-fourth/30 backdrop-blur-xl rounded-lg border border-slate-600 flex flex-col overflow-hidden">
                      <div className="flex justify-between items-center px-4 py-2" onClick={() => handleShow(5)}>
                        <h1 className="text-sm text-white font-semibold">Convenience Store</h1>
                        <span className={`text-lg text-white transform transition-transform duration-300 ${show === 5 ? "rotate-180" : "rotate-0"}`}>
                    ...
                    <div className="w-full min-h-10 bg-fourth/30 backdrop-blur-xl rounded-lg border border-slate-600 flex flex-col overflow-hidden">
                      <div className="flex justify-between items-center px-4 py-2" onClick={() => handleShow(6)}>
                        <h1 className="text-sm text-white font-semibold">Bank</h1>
                        <span className={`text-lg text-white transform transition-transform duration-300 ${show === 6 ? "rotate-180" : "rotate-0"}`}>
        ...
        <div className="w-full min-h-96 flex flex-col gap-2">
            ...
            {groupedFields.length > 1 ? (
              <>
                <div className="flex items-center px-4">
                  ...
                  <div className="w-[60%] flex items-center h-7 sm:h-8">
                    <h1 className="text-xs sm:text-sm text-white">{selected.userId}</h1>
                  </div>
                </div>
                <div className="flex items-center px-4">
                  <div className="w-[40%] flex items-center h-7 sm:h-8 ">
                  ...
                  <div className="w-[60%] flex items-center h-7 sm:h-8">
                    <h1 className="text-xs sm:text-sm text-white">{selected.zoneId}</h1>
                  </div>
                </div>
              </>
            ) : groupedFields.length === 1 ? (
          ...
          <div className="flex items-center gap-2 mt-3 sm:mt-7">
            <button
              ...
              disabled={nickname === "Error" || nickname === "ID tidak ditemukan" || nickname === "Category tidak ditemukan"}
              onClick={() => order()}>
              Pesan Sekarang
            </button>
            <button
