...
const CoverHeader = ({ data, features }) => {
  return (
    <div className="relative w-full h-[21rem] xl:h-[35rem] mb-10 flex flex-col overflow-hidden">
      ...
      {/* Cover Content */}
      <div className="relative w-full bg-fourth h-[8.5rem] xl:h-40">
        <div className="absolute -bottom-16 left-40 md:left-[10.5rem] lg:left-[13.5rem] xl:-bottom-11 xl:left-[26rem] w-full h-[12.5rem] z-10 py-2 flex flex-col gap-3">
          <div className="">
            <h1 className="text-white text-xl font-bold">{data.product.title}</h1>
            <h1 className="text-white text-md">{data.product.subTitle}</h1>
          </div>
          <div
            className="absolute top-[4.5rem] -left-36 sm:top-20 md:static 
          ...
          >
            {features
              .map((item, index) => (
                <h1
                  key={index}
                  className="text-mini text-zs xl:text-sm text-white font-semibold"
                >
                  {item.feature}{" "}
                  <span className="mx-2 ml-2 mr-0">
